...
static void test_perceptual_diff_identical()
{
	...
	gdTestAssertMsg(result.pixels_changed == 0, "Expected 0 pixels changed, but got %d", result.pixels_changed);
...
static void test_perceptual_diff_black_vs_white()
{
    ...
    gdTestAssertMsg(result.pixels_changed == 64 * 64, "Expected %d pixels changed, but got %d", 64 * 64, result.pixels_changed);
...
{
    /* two colors that differ by 1 in each channel — well below any sane threshold */
...
static void test_perceptual_diff_alpha_delta()
{
    ...
    gdImagePtr img2 = make_solid(32, 32, 200, 100, 50, 127); /* fully transparent */
    CuTestImageResult result = {0, 0};

    gdTestImagePerceptualDiff(img1, img2, NULL, &result, 0.1);

    gdTestAssertMsg(result.pixels_changed == 32 * 32, "Expected %d pixels changed, but got %d", 32 * 32, result.pixels_changed);
...
static void test_perceptual_diff_image_output()
{
    ...
    for (y = 0; y < 4; y++) {
        for (x = 0; x < 4; x++) {
            ...
            gdTestAssertMsg(gdTrueColorGetRed(p) == 255, "Expected red 255, but got %d", gdTrueColorGetRed(p));
            gdTestAssertMsg(gdTrueColorGetGreen(p) == 0, "Expected green 0, but got %d", gdTrueColorGetGreen(p));
            gdTestAssertMsg(gdTrueColorGetBlue(p) == 0, "Expected blue 0, but got %d", gdTrueColorGetBlue(p));
            gdTestAssertMsg(gdTrueColorGetAlpha(p) == 0, "Expected alpha 0, but got %d", gdTrueColorGetAlpha(p));
        }
    }
