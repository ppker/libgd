...
static void assert_bbox(const char *label, gdRectPtr src,
						const double affine[6], int x, int y, int width,
						int height) {
	gdRect bbox;

	gdTestAssertMsg(gdTransformAffineBoundingBox(src, affine, &bbox) == GD_TRUE,
					"%s: expected bounding box calculation to succeed", label);
	gdTestAssertMsg(bbox.x == x && bbox.y == y && bbox.width == width &&
						bbox.height == height,
					"%s: expected bbox {%d,%d,%d,%d}, got {%d,%d,%d,%d}", label,
					x, y, width, height, bbox.x, bbox.y, bbox.width,
					bbox.height);
}

static void assert_getimage_dims(const char *label, gdImagePtr src,
								 gdRectPtr src_region, const double affine[6],
								 int width, int height) {
	gdImagePtr dst = NULL;

	gdTestAssertMsg(gdTransformAffineGetImage(&dst, src, src_region, affine) ==
						GD_TRUE,
	...
	if (dst != NULL) {
		gdTestAssertMsg(gdImageSX(dst) == width && gdImageSY(dst) == height,
						"%s: expected output %dx%d, got %dx%d", label, width,
						height, gdImageSX(dst), gdImageSY(dst));
		gdImageDestroy(dst);
	}
}
...
static void test_matrix_bounding_boxes_and_images(void) {
	...
	assert_getimage_dims("nonuniform scale", src, &rect, nonuniform_scale, 16,
						 9);
	...
	gdTestAssertMsg(gdImageSetInterpolationMethod(src, GD_CATMULLROM) ==
						GD_TRUE,
					"expected interpolation method setup to succeed");
	old_m = gdImageGetInterpolationMethod(src);
	gdTestAssertMsg(
		gdTransformAffineBoundingBox(&rect, mixed, &bbox) == GD_TRUE,
		"mixed transform: expected bounding box calculation to succeed");
	gdTestAssertMsg(bbox.width > 0 && bbox.height > 0,
					"mixed transform: expected positive bbox, got %dx%d",
					bbox.width, bbox.height);
	assert_getimage_dims("mixed transform", src, &rect, mixed, bbox.width,
						 bbox.height);
	new_m = gdImageGetInterpolationMethod(src);
	gdTestAssertMsg(
		new_m == old_m,
		"mixed transform: expected interpolation method %d to remain %d", new_m,
		old_m);
...
static void test_clipped_region_does_not_read_neighboring_pixels(void) {
	...
	gdAffineScale(scale, 2.0, 2.0);
	gdTestAssertMsg(
		gdTransformAffineGetImage(&dst, src, &region, scale) == GD_TRUE,
		"clipped region: expected affine image creation to succeed");
	gdTestAssertMsg(dst != NULL,
					"clipped region: expected affine output image");
	if (dst != NULL) {
		px = gdImageGetTrueColorPixel(dst, 0, 0);
		gdTestAssertMsg(gdTrueColorGetRed(px) == 0,
						"clipped region: expected no red leakage from outside "
						"src_region, got %x",
						px);
		gdTestAssertMsg(gdTrueColorGetGreen(px) > 0,
						"clipped region: expected green contribution from "
						"inside src_region, got %x",
						px);
		gdTestAssertMsg(gdTrueColorGetAlpha(px) > gdAlphaOpaque,
						"clipped region: expected edge sample to blend with "
						"transparent outside, got %x",
						px);
		gdImageDestroy(dst);
	}
...
static void
test_affine_copy_preserves_destination_for_transparent_samples(void) {
	gdImagePtr src, dst;
	gdRect rect = {0, 0, 4, 4};
	double identity[6];
	...
	gdAffineIdentity(identity);
	gdTestAssertMsg(gdTransformAffineCopy(dst, 2, 2, src, &rect, identity) ==
						GD_TRUE,
					"copy transparent: expected affine copy to succeed");
	px = gdImageGetTrueColorPixel(dst, 2, 2);
	gdTestAssertMsg(
		px == blue,
		"copy transparent: expected destination pixel to be preserved, got %x",
		px);
...
static void test_palette_source_transform(void) {
	...
	gdTestAssertMsg(
		gdTransformAffineGetImage(&dst, src, &rect, shear) == GD_TRUE,
		"palette source: expected affine image creation to succeed");
	gdTestAssertMsg(dst != NULL,
					"palette source: expected affine output image");
	if (dst != NULL) {
