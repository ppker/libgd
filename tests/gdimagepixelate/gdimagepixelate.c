...
static const int expected_upperleft[][3] = {{0x000000, 0x040404, 0x080808},
...
#define CHECK_PIXELS(im, expected)                                             \
	do {                                                                       \
		int x, y;                                                              \
		for (y = 0; y < (im)->sy; y++) {                                       \
			for (x = 0; x < (im)->sx; x++) {                                   \
				...
				int r =                                                        \
					((expected)[y / BLOCK_SIZE][x / BLOCK_SIZE] >> 16) & 0xFF; \
				int g =                                                        \
					((expected)[y / BLOCK_SIZE][x / BLOCK_SIZE] >> 8) & 0xFF;  \
...
static int testPixelate(gdImagePtr im) {
	if (gdImagePixelate(im, -1, GD_PIXELATE_UPPERLEFT) != 0)
		return 0;
	if (gdImagePixelate(im, 1, GD_PIXELATE_UPPERLEFT) != 1)
		return 0;
	if (gdImagePixelate(im, 2, -1) != 0)
		return 0;

	SETUP_PIXELS(im);
	if (!gdImagePixelate(im, BLOCK_SIZE, GD_PIXELATE_UPPERLEFT))
		return 0;
	CHECK_PIXELS(im, expected_upperleft);

	SETUP_PIXELS(im);
	if (!gdImagePixelate(im, BLOCK_SIZE, GD_PIXELATE_AVERAGE))
		return 0;
