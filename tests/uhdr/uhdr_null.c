...
int main() {
	...
	gdTestAssertMsg(gdUhdrIsAvailable() == 1,
					"UltraHDR support should be enabled for this test\n");

	gdTestAssertMsg(
		gdUhdrImageCreateFromFile(NULL, GD_UHDR_FORMAT_JPEG, &err) == NULL,
	...
	gdTestAssertMsg(rc == GD_UHDR_E_INVALID,
					"gdUhdrImageResize(NULL, ...) should return "
					"GD_UHDR_E_INVALID, got %d\n",
					rc);

	memset(&err, 0, sizeof(err));
	rc = gdUhdrImageRotate(NULL, 90, &err);
	gdTestAssertMsg(rc == GD_UHDR_E_INVALID,
					"gdUhdrImageRotate(NULL, ...) should return "
					"GD_UHDR_E_INVALID, got %d\n",
					rc);

	memset(&err, 0, sizeof(err));
	gdTestAssertMsg(
		gdUhdrImageCreateFromPtr(0, NULL, GD_UHDR_FORMAT_JPEG, &err) == NULL,
