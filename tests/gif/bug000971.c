...
int main() {
	gdImagePtr redFrame = NULL, greenFrame1 = NULL, greenFrame2 = NULL,
			   blueFrame = NULL;
	void *begin = NULL, *red = NULL, *green1 = NULL, *green2 = NULL,
		 *blue = NULL, *end = NULL;
	int begin_size = 0, red_size = 0, green1_size = 0, green2_size = 0,
	...
	if (!gdTestAssert(redFrame != NULL && greenFrame1 != NULL &&
					  greenFrame2 != NULL && blueFrame != NULL)) {
		goto done;
	}

	begin = gdImageGifAnimBeginPtr(redFrame, &begin_size, 0, 0);
	red = gdImageGifAnimAddPtr(redFrame, &red_size, 1, 0, 0, 100,
							   gdDisposalNone, NULL);
	green1 = gdImageGifAnimAddPtr(greenFrame1, &green1_size, 1, 0, 0, 100,
								  gdDisposalNone, redFrame);
	green2 = gdImageGifAnimAddPtr(greenFrame2, &green2_size, 1, 0, 0, 100,
								  gdDisposalNone, greenFrame1);
	blue = gdImageGifAnimAddPtr(blueFrame, &blue_size, 1, 0, 0, 100,
								gdDisposalNone, greenFrame2);
	...
	gdTestAssertMsg(green2 != NULL,
					"expected duplicate green placeholder frame data");
	gdTestAssertMsg(blue != NULL, "expected blue frame data");
	gdTestAssertMsg(end != NULL, "expected GIF animation terminator data");
	if (begin == NULL || red == NULL || green1 == NULL || green2 == NULL ||
		blue == NULL || end == NULL) {
		goto done;
	}

	total_size = begin_size + red_size + green1_size + green2_size + blue_size +
				 end_size;
	...
	memcpy(gif + begin_size + red_size + green1_size + green2_size, blue,
		   blue_size);
	...
	gdTestAssertMsg(info.one_by_one_frames == 1,
					"expected one 1x1 transparent placeholder frame, got %d",
					info.one_by_one_frames);
