...
void save(gdImagePtr im, const char *filename) {
...
void do_test() {
	...
	/* Ditto a black image. */
	mkblack(im);
	gdTestAssert(gdMaxPixelDiff(im, imref) >=
				 240); /* Leaves a little wiggle room */
...
void do_crosstest() {
	...
	gdTestAssert(whitecmp(im, blurred, WIDTH - 5, HEIGHT - 5) <=
				 PIXEL_CLOSE_ENOUGH);

	//    printf("%d %d %d %d\n", whitecmp(im, blurred, 0, 0), whitecmp(im,
	//    blurred, WIDTH-1, 0),
	//         whitecmp(im, blurred, 0, HEIGHT-1), whitecmp(im, blurred,
	//         WIDTH-1, HEIGHT-1));
	...
	gdTestAssert(whitecmp(im, blurred, WIDTH - 1, HEIGHT - 1) <=
				 PIXEL_CLOSE_ENOUGH);
