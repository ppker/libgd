...
int main() {
	...
	if (!gdTestAssertMsg(heif_get_version_number_major() == 1 &&
							 heif_get_version_number_minor() >= 9,
						 "changing chroma subsampling is not supported in this "
						 "libheif version\n"))
		return 77;
	...
	if (!gdTestAssertMsg(
			...
			"HEVC codec support missing from libheif\n"))
		return 77;
	...
	p = gdImageHeifPtrEx(src, &size, 200, GD_HEIF_CODEC_HEVC,
						 GD_HEIF_CHROMA_444);
	gdTestAssertMsg(p != NULL, "return value of gdImageHeifPtrEx() is null\n");
	gdTestAssertMsg(size > 0,
					"gdImageHeifPtrEx() output size is non-positive\n");

	dst = gdImageCreateFromHeifPtr(size, p);
	gdTestAssertMsg(dst != NULL,
					"return value of gdImageCreateFromHeifPtr() is null\n");

	if (gdTestAssertMsg(src != NULL && dst != NULL,
						"cannot compare with NULL buffer"))
