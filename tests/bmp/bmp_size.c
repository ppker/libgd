...
int main() {
	...
	gdTestAssertMsg(bfSize == (unsigned int)size, "expected %d, got %u", size,
					bfSize);
	bfOffBits = get_field(data, OFFSET_BF_OFF_BITS);
	biSizeImage = get_field(data, OFFSET_BI_SIZE_IMAGE);
	gdTestAssertMsg(biSizeImage == (unsigned int)size - bfOffBits,
					"expected %u, got %u", size - bfOffBits, biSizeImage);
	...
	biSizeImage = get_field(data, OFFSET_BI_SIZE_IMAGE);
	gdTestAssertMsg(biSizeImage == (unsigned int)size - bfOffBits,
					"expected %u, got %u", (unsigned int)size - bfOffBits,
					biSizeImage);
