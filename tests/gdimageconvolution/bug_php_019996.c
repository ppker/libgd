...
int main() {
	...
	int x, y;
	float matrix[3][3] = {
		{1.0f, 2.0f, 1.0f}, {2.0f, 4.0f, 2.0f}, {1.0f, 2.0f, -INFINITY}};
	 ...
	 * With gcc/clang, to see warnings, if regression is present,
	...
	for (y = 0; y < gdImageSY(im); y++) {
		for (x = 0; x < gdImageSX(im); x++) {
			...
			gdTestAssertMsg(
				gdTrueColorGetRed(px) == 0,
				"expected red channel to clamp to 0 at (%d,%d), got %d", x, y,
				gdTrueColorGetRed(px));
			gdTestAssertMsg(
				gdTrueColorGetGreen(px) == 0,
				"expected green channel to clamp to 0 at (%d,%d), got %d", x, y,
				gdTrueColorGetGreen(px));
			gdTestAssertMsg(
				gdTrueColorGetBlue(px) == 0,
				"expected blue channel to clamp to 0 at (%d,%d), got %d", x, y,
				gdTrueColorGetBlue(px));
			gdTestAssertMsg(
				gdTrueColorGetAlpha(px) == gdAlphaOpaque,
				"expected alpha to remain opaque at (%d,%d), got %d", x, y,
				gdTrueColorGetAlpha(px));
		}
	}
